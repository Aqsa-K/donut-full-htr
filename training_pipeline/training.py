...
# We can also parse the string as a Python dictionary using ast.literal_eval. 
# Each training example has a single "gt_parse" key, which contains the ground truth parsing of the document:
...
# Set the image size and max length for the encoder-decoder model
image_size = [800, 512]
max_length = 768
...
# Add special tokens to the tokenizer and resize the model's token embeddings
...
# prompt: create a directory 'checkpoint' if doesnt already exist 
